...
print "SEQUENCE_START %s" % (sequenceKey, )
#sender.sendStreamItem("CALPANEL %s" % (sequenceKey, ) )
print "CALPANEL %s" % (sequenceKey, )
...
if True:
    ...
    print "CUBES_ONLY %s" % (sequenceKey, )
    for N in range(0, len(cubelist)):
        #sender.sendStreamItem("CUBE %s xcal=%s ycal=%s rcal=%s time=%i" % (sequenceKey, calx, caly, calR, cubetimes[N], ))
        print "CUBE %s xcal=%s ycal=%s rcal=%s time=%i" % (sequenceKey, calx, caly, calR, cubetimes[N], )
...
print "SEQUENCE_END %s" % (sequenceKey, )
